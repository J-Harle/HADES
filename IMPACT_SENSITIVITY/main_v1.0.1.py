...
# Read log files from the 'PM7' subdirectory
def read_log_files():
    """
    Retrieves all .log files from the 'PM7' subdirectory.
    Returns:
        tuple: List of log file paths and the subdirectory path.
    """
...
# Parse log files to extract frequencies, normal modes, and atomic coordinates
def get_file_data(log_files):
    """
    Parses .log files to extract vibrational frequencies, normal modes, and atomic coordinates.
    Args:
        log_files (list): List of paths to .log files.
    Returns:
        list: List of tuples containing (filename, atom_count, frequencies, atoms, normal_modes).
    """
    file_data = []
    for log_file in log_files:
        ...
        # Extract atomic coordinates from the last 'Standard orientation' section
        std_indices = [i for i, l in enumerate(lines) if "Standard orientation:" in l]
        if std_indices:
            last_std = std_indices[-1] # Finds the last instance of "standard orientation"
            atom_start = last_std + 5 # Atom start occurs five lines after std orientation
            atoms = []
            for line in lines[atom_start:]:
                if "----" in line: break
                parts = line.split()
                if len(parts) < 6: continue # Center Number | atomic number | atomic type | x | y | z
                try:
        ...
        while i < len(lines):
            if "Frequencies --" in lines[i]:
                freq_parts = lines[i].split()[2:] # Frequencies | -- | Freq 1 | Freq 2 | Freq 3
                ...
                try:
                    ...
                    i += 1
                    continue
                j = i + 1 # Eigenvector data starts after frequencies
                ...
                while j < len(lines) and not lines[j].strip():
                    j += 1
                e_vectors = [np.zeros((atom_count, 3)) for _ in range(num_modes_this_block)]
                atom_idx = 0
                expected_columns = 2 + 3 * num_modes_this_block 
                # ^ expects Atom | AN | dx1 | dy1 | dz1 | dx2 | dy2 | dz2 | ... |dz3
                while j < len(lines) and atom_idx < atom_count:
                    ...
                    try:
                        ...
                        for m in range(num_modes_this_block):
                            offset = 2 + m * 3 # First eigenvector starts at index 2, 2nd at 5 and 3rd at 8.
                            dx = float(parts[offset]) # dx values at parts[2, 5, 8]
                            dy = float(parts[offset + 1]) # dy values at parts[3, 6, 9]
                            dz = float(parts[offset + 2]) # dz values at parts[4, 7, 10]
                            e_vectors[m][atom_idx] = [dx, dy, dz]
                    except (ValueError, IndexError) as e:
...
def gaussian_broadening(histogram, bwidth=0.5, gwidth=2.5):
    ...
    Args:
        ...
        bwidth (float): Bin width for the histogram. # magic number
    ...
    if gwidth < bwidth:
        ...
        for i in range(-int(3.0 * gwidth / bwidth), int(3.0 * gwidth / bwidth) + 1): # +/- 3 FWHM
...
def generate_box_dos(freq_axis, dos, omega_max):
    ...
    box_dos = np.copy(dos)
    box_area = 6
    intensity = box_area / omega_max # Calculat the intensity of the box
...
def bose_einstein_dos(box_dos, freq_axis):
    ...
    for i, w in enumerate(freq_axis):
        if w > 40: 
            n = (1.0 / (np.exp(hbar * (w*(2*math.pi) * (1E9 * 29.979245)) / (k * t)) - 1.0)) # unitless
...
def plot_first_convolution(freq_axis, first_convolved_dos, first_conv_no_proj, be_dos, filename, atom_count, dpi, base_dir):
    ...
    # ax1.plot(freq_axis, first_conv_no_proj, label="First Convolution (Unprojected)", color="#000000")
...
# This function works, but it outdated and still expects multiple up-pumped values per molecule
...
def plane_basis_from_axis(u):
    ...
    e1 = unit(e1) # Dot product to get the first basis vector
    e2 = np.cross(u, e1) # Cross product to get the second basis vector
    # Both e1 and e2 are unit vectors and orthogonal to u
    return e1, e2
...
def angle_in_fixed_plane(point, origin, u_axis, e1, e2):
    ...
    """
    O = np.asarray(origin, float) # Origin is always the coordinates of the nitrogen 
    P = np.asarray(point, float) # Point is always the coordinates of one of the oxygens
    u = unit(u_axis)
    r = P - O
...
def displace_along_mode(coords, mode_vec, amplitude_angstrom, masses_amu=None, already_mass_weighted=True):
    ...
    # Gaussian 16 outputs mass-weighted modes, but must be unweighted for displacement.
    Args:
...
def no2_angle_changes_for_mode(group, atoms, mode_vec, amplitude_angstrom=1, already_mass_weighted=True):
    ...
    Essentially, this is just looking down the U vector to just see the e1 and e2 vectors.
    Args:
...
if __name__ == "__main__":
    ...
    for filename, atom_count, frequencies, atoms, normal_modes in freq_data:
        ...
        if not no2_groups:
            ...
            min_angle_deg = mad
            valid_modes = []
            all_modes = []  # To store all modes with valid angle changes
            for i, mode_vec in enumerate(normal_modes):
                if i >= len(frequencies):
                ...
                for group in no2_groups:
                    ...
                    if dtheta > mode_max_dtheta:
                        mode_max_dtheta = dtheta
                mode_max_dtheta_deg = np.degrees(mode_max_dtheta)
                if mode_max_dtheta > 0:  # Store all modes with non-zero, non-NaN angle changes
                    all_modes.append((freq, mode_max_dtheta_deg))
                if mode_max_dtheta_deg >= min_angle_deg:
            ...
            if valid_modes:
                ...
                # Select the mode with the largest angle change
                omega_max, angle_at_omega_max = max(all_modes, key=lambda x: x[1])
                print(f"No NO2 angle change > {min_angle_deg}° in bounds for {molecule_name}, "
                      f"using frequency with largest angle change: ω_max = {omega_max:.1f} cm^-1 "
                      f"(Δθ = {angle_at_omega_max:.2f}°)")
            else:
                print(f"No valid NO2 angle changes in bounds for {molecule_name}, using default ω_max")
                omega_max = 200
