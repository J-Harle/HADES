...
parser = argparse.ArgumentParser(description="Calculate oxygen balance and write to CSV")
...
def oxygen_balance(C, H, O, mol_weight):
    ...
    to CO2 and H2O. Negative values indicate oxygen deficiency.
...
def process_xyz_files(parent_dir=None, output_csv="hades_out.csv"):
    """
    ...
    if os.path.isfile(abs_output):
        try:
            with open(abs_output, "r", newline="") as csvfile:
                reader = csv.DictReader(csvfile)
                for row in reader:
                    # normalise filename (without extension)
                    existing_filenames.add(row.get("Filename", "").strip())
        except Exception as e:
            print(f"WARNING: Could not read existing CSV '{abs_output}': {e}")
    ...
    try:
        with open(abs_output, "a", newline="") as csvfile:
            ...
            for root, dirs, files in os.walk(parent_dir):
                for file in files:
                    if not file.lower().endswith(".xyz") or file.startswith("."):
                        continue

                    base = file.split(".")[0]
                    if base in existing_filenames:
                        continue
            ...
            if not found_any:
                print("NOTICE: No new .xyz files were found under the parent directory.")
...
if __name__ == "__main__":
    process_xyz_files(output_csv=args.input)
