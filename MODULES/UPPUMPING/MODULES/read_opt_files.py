...
def read_extended_csv(
    ...
    with open(csv_path, "r", encoding="latin-1") as f:
        ...
        for row in reader:
            ...
            filename = row["FILENAME"].upper()

            if filter_cluster_id is not None and cluster_id not in filter_cluster_id:
                continue
...
def read_txt_xyz(molecules, base_dir):
    data = []
    ...
    for dirname in tqdm(sorted(valid_dirs), desc="Reading Data Files", unit="molecule"):
        ...
        freqs, all_freqs, eigenvectors, coordinates = [], [], [], []
        atom_count = None
...
if __name__ == "__main__":
    ...
    runs = [
        {
            "csv": "Storm_dataset.csv",
            "path": ("..", "OPTIMISED_STRUCTURES", "STORM", "SMALL_MODEL"),
            # "filter_cluster_id": None
            "filter_cluster_id": ["nitroaliphatic", "nitroaromatic", "nitramine", 
                                  "nitrate ester", 
                                  "nitro-1,2,4-triazole", "nitropyrazole",
                                  "nitroimidazole", "nitropyrimidine", "nitropyridine"
                                  "nitro-1,2,3-triazole", "picryl"

                                #   "nitrofuroxan",
                                  "amine",
                                  "nitroaromatic with C-H linkage"],
                                # Removed:
                                #  "picryl"
                                #  "nitroaliphatic containing other functional groups",
                                #  "azo",
                                #  "terphenyl"
        ...
        {
            "csv": "30_bench.csv",
            "path": ("..", "OPTIMISED_STRUCTURES", "30_MOL"),
            "filter_cluster_id": None
        }
    ]
    ...
    for run in runs:
        ...
        print(f"\nRunning dataset: {run['csv']}")

        base_dir = os.path.abspath(
            os.path.join(script_dir, *run["path"])
        )

        molecules = read_extended_csv(
            csv_filename=run["csv"],
            use_clustering=True,
            filter_cluster_id=run["filter_cluster_id"]
        )
        ...
        data = read_txt_xyz(flat_molecules, base_dir)

        output_name = run["csv"].replace(".csv", "_raw.csv")
        write_data_to_csv(data, output_name)
