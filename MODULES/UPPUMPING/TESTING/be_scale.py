...
def compute_thermal_vibrational_energy(all_results):
    hbar = scipy.constants.hbar
    k = scipy.constants.k
    t = 300
    ...
    for mol in all_results:
        ...
        for i, w in enumerate(freq_axis):
            ...
            omega = w * (2 * math.pi) * (1E9 * 29.979245)

            n = 1.0 / (np.exp(hbar * omega / (k * t)) - 1.0)

            mode_energy = hbar * omega * n

            total_energy += dos[i] * mode_energy

        mol["thermal_vibrational_energy_J"] = total_energy
