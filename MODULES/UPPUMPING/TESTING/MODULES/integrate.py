...
def integrate(all_data):
    ...
    for mol in tqdm(all_data, desc="Calculating Integral", unit="molecule"):
        ...
        if not np.any(mask):
            print(f"[WARN] No data in ωmax–3ωmax window for {name}")
            mol["raw_integral"] = None
            mol["integral_over_freq_count"] = None
            mol["exp_ratio_integral"] = None
            continue
...
if __name__ == "__main__":
    ...
    # Write results back to CSV (original CSV will be overwritten)
    write_updated_csv(all_data, "raw.csv")

    print(all_data[0].keys())

    # print(f"[INFO] Integrals calculated for {len(all_data)} molecules")
