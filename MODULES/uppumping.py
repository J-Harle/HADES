...
def plot_max_min_normalised(i50_values, integral_values, molecule_omega_max, freq_data, molecule_bounds, dpi, base_dir):
    ...
    sorted_molecule_names = sorted(
        {name for name in i50_values.keys() if name in integral_values},
        key=lambda name: i50_values[name][0]
    )
    ...
    # --- [Linear fit on 1/I50] ---
    inv_i50 = 1.0 / np.array(i50_for_plot)
    ...
    print("\n" + "="*60)
    print("LINE OF BEST FIT (normalized_integral vs 1/I₅₀)")
    print("="*60)
    print(f"  Equation:  y = {a:.6e} × (1/I₅₀) + {b:.6e}")
    print(f"  R²       :  {r_squared:.4f}")
    print(f"  N        :  {len(i50_for_plot)} molecules")
    print(f"  Use this to predict I₅₀ from new normalized integral (y):")
    print(f"      I₅₀ = 1 / ((y - {b:.6e}) / {a:.6e})")
    print("="*60 + "\n")

    # --- [Plotting] ---
    fig, ax = plt.subplots(figsize=(12, 9), dpi=dpi)
    ax.plot(i50_for_plot, avg_normalised_integrals, 'o', markersize=6, color='tab:green')
    ...
    ax.set_xlabel("I₅₀ Value / Nm", fontsize=24)
...
if __name__ == "__main__":
    ...
    freq_data, subdir = get_file_data_from_txt_xyz(subdir="UPPUMPING/OPTIMISED_STRUCTURES/SMALL_MODEL")
