...
def second_convolution(all_data):
    for mol in tqdm(all_data, desc="Calculating Second Convolution", unit="molecule"):
        ...
        exp_be_dos = np.array(mol["exp_be_dos"])
    ...
    for row in all_rows:
        try:
            data.append({
                ...
                "exp_be_dos": json.loads(row["exp_be_dos"]) if row.get("exp_be_dos") else [],
...
if __name__ == "__main__":
    ...
    for csv_name in runs:
        ...
        # Load CSV
        all_rows, conv_data = read_csv_for_convolutions(csv_name)
        ...
        write_updated_csv(all_rows, conv_data, csv_name)
